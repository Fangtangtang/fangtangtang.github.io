...
We also assume that the maximum sum of simultaneous processes allocation requirement is $w^*$. 
Note that $OPT\ge w^*$.
Also note that in the graph we introduced in Section 1, $k$ is the maximum clique size and $w^*$ is the maximum weight sum of all cliques.
...
**Claim 1.** Let $Z(i)=\frac{w^*}{i \ln 2}$, all requests of size $j$ will be allocated before address $\sum_{i=1}^{j}Z(i)$.

**Proof.**
We prove by induction.
When $j=1$, $\sum_{i=1}^{j}Z(i)=Z(1)=\frac{w^*}{\ln 2}>w^*$. By the definition of $w^*$, the request of size $1$ will be allocated before address $w^*$.
...
\begin{aligned}
  S &= \sum_{j=0}^{s-1}\frac{(D(j+1)-D(j))(j+1)}{j+s+1} + \frac{(\sum_{i=1}^{s+1}Z(i)-(s+1)-D(s))(s+1)}{2s+1}\\
    ...
    &= \frac{w^*}{\ln 2}\sum_{j=0}^{s-1}\frac{1}{j+s+1}  + (s+1)\frac{Z(s+1)-(s+1)}{2s+1}\\
    &> w^* - (s+1)
\end{aligned}
$$

This means that when we want to allocate memory for a request of size $s+1$ which fails our claim, current occupied memory space exceeds $w^* - (s+1)$.
And this leads to a contradiction to the definition of $w^*$. So the claim holds for $j=s+1$.

$\blacksquare$
{: .qed}

The $OPT$ is at least $w^*$.
By Claim 1, $ALG\le \sum_{i=1}^{W_{max}}Z(i)=\frac{w^*}{\ln 2}\sum_{i=1}^{W_{max}}\frac{1}{i}$.
...
> - Occupied memory slice are scattered in the memory space, but the sum of these slice do not exceed $w^*$, which is the maximum simultaneously allocated memory.

**Lemma 1.** Let $c$ be a chosen constant, address higher than $(c+1)\times w^*$ will not be allocated to requests smaller $\le c\frac{W_{max}}{k}$.

**Proof.**
We prove this by contradiction. Assume that there exists a requests $e$ with size $w_e \le c\frac{W_{max}}{k}$ that is allocated at $Addr > (c+1)\times w^*$, 
...
Since $W_{max}\le w^*$, the wasted space $S_{wasted}<cw^*$.
By the definition of $w^*$, the used space is at most $w^*$, so the used space before $Addr$ has $S_{used}\le w^*$.
So the space before $Addr$ is $S_{wasted}+S_{used}<(c+1)\times w^*$, which contradicts the fact that $Addr$ is higher than $(c+1)\times w^*$.
...
**Claim 2.** A request of size $2^j, j\ge l$ can be allocated at an address not exceeding $(c+2+j-l)\times w^*$.

**Proof.**
We start with request of size $2^l$. 
We learn from Lemma 1 that after address $(c+1)\times w^*$, the allocated size for each request is at least $2^l$.
...
By the definition of $w^*$, there can not be a continuous memory space $>w^*$ which is fully occupied.
So the request of size $2^l$ can at least fit in a fragment before exceeding $(c+2)\times w^*$.

For $j=l+1$, since the fragment above $(c+1)\times w^*$ is at least $2^l$ and since there are $2w^*$ between $(c+1)\times w^*$ and $(c+3)\times w^*$, 
we can find at least one fragment or continuous free space that $2^j$ can fit in, otherwise the portion of fragment and occupied space will lead to a contradiction with the definition of $w^*$. 
Other induction steps are similar.

$\blacksquare$
{: .qed}

We can derive from Claim 2 that the competitive ratio do not exceed $\frac{(c+2+j_{max}-l)\times w^*}{w^*}=c+2+j_{max}-l$, 
...
$$
5w^*+\frac{w^*}{0.3}\sum_{i= \frac{W_{max}}{k} + 1}^{j}\frac{1}{i}
$$
...
$$
L_i=(c+1)w^*+\frac{w^*}{0.3}\sum_{k= \frac{W_{max}}{k} + 1}^{i}\frac{1}{k}
$$
...
$$
\sum_{i=\frac{W_{max}}{k} + 1}^{j+1}\frac{i}{i+j}(L_i-L_{i-1})=\frac{w^*}{0.3}\sum_{i=\frac{W_{max}}{k} + 1}^{j+1}\frac{1}{i+j}=\frac{w^*}{0.3} \sum_{i=\frac{W_{max}}{k} +j+ 1}^{2j+1}\frac{1}{i}
$$
...
$$
\frac{w^*}{0.3} \sum_{i=\frac{W_{max}}{k} +j+ 1}^{2j+1}\frac{1}{i}\ge w^*
$$

which contradicts the definition of $w^*$. So $L_{j+1}$ is the upper bound as we chose $c=4$.
...
$$
M\le 5w^*+\frac{w^*}{0.3}\sum_{i=\frac{W_{max}}{k} + 1}^{W_{max}}\frac{1}{i}\le 5w^*+\frac{w^*}{0.3}\left(1+\ln\left(\frac{W_{max}}{1+\frac{W_{max}}{k} }\right)\right)\le w^*(9+4\ln k)
$$
...
For each type, the number of segments assigned to is is $k_{t_i}$, so the total memory assigned to it is $k_{t_i}\times 2^{t_i}\le 2w^*$.

Since we classify by request size and the maximum size is $W_{max}$, there are at most $\lceil \log_2 (W_{max}) \rceil$ types.
So $ALG\le \lceil \log_2 (W_{max}) \rceil \times 2 \times w^*$. So the competitive ratio is $O(\log(W_{max}))$.
...
2. Type $t_i$ satisfying $2^{t_i}\ge \frac{W_{max}}{k}$.
   ...
   The total memory space needed by this kind is at most $2w^*\times num$, $num$ is the number of types of this kind.
   Since $2^{t_i}\ge \frac{W_{max}}{k}$, $num$ is $O(\log(k))$.
   With fixed constant $c$ such that $num \le c \cdot \log(k)$,  $2w^*\times num\le c \cdot \log(k)\cdot 2w^*$. 

So $ALG\le 2W_{max}+c \cdot \log(k)\cdot 2w^*$. 
The competitive ratio is $\frac{ALG}{OPT}=2c\cdot \log(k)+\frac{2W_{max}}{w^*}$, can be bounded by $O(\log k)$.
...
Robson [6] indicated that with *Buddy System*, a memory of size $2w^*\log_2(W_{max})$ is enough to serve the requests with distribution we discussed in these notes.
